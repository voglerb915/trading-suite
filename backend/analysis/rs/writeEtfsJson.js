...
// 🟢 Holt die Daten aus deiner ETF-Pipeline
...
function loadHistoryFiles() {
  ...
  return fs.readdirSync(historyDir)
    .filter(f => f.endsWith('_etfs.json')) // 🟢 Wichtig: _etfs.json filtern!
    .sort()
    .map(f => ({
      date: f.slice(0, 10),
...
async function writeEtfsJson() {
  let etfs = await buildEtfRsSnapshot();
  if (!etfs || etfs.length === 0) return etfs;

  const latestDate = normalizeDate(etfs[0].anl_datum);
  etfs = etfs.filter(e => normalizeDate(e.anl_datum) === latestDate);

  // 🟢 Speicherpfade für ETFs anpassen
  const snapshotFile = path.join(__dirname, '..', '..', 'json', 'rs_etfs.json');
  const historyDir = path.join(__dirname, '..', '..', 'json-history');
  if (!fs.existsSync(historyDir)) fs.mkdirSync(historyDir, { recursive: true });

  const historyFile = path.join(historyDir, `${latestDate}_etfs.json`);

  // Roh-Snapshot speichern
  fs.writeFileSync(historyFile, JSON.stringify(etfs, null, 2));

  // Diffs berechnen
  const historyFiles = loadHistoryFiles();
  const etfsWithDiffs = computeDiffs(etfs, historyFiles);

  // ---------------------------------------------------------
  // marketScores: Insert ETFs into DB
  // ---------------------------------------------------------
  ...
  for (const item of etfsWithDiffs) {
      await pool.request()
          .input('type', sql.VarChar, 'etf') // 🟢 Wichtig: Typ ist hier 'etf'
          .input('name', sql.VarChar, item.ticker)
  ...
  console.log(`📥 marketScores: ${etfsWithDiffs.length} ETF‑Einträge gespeichert`);

  // Snapshot + History schreiben
  fs.writeFileSync(snapshotFile, JSON.stringify(etfsWithDiffs, null, 2));
  fs.writeFileSync(historyFile, JSON.stringify(etfsWithDiffs, null, 2));

  console.log(`✅ RS ETFs Snapshot + History geschrieben (${etfs.length} Einträge, Datum ${latestDate})`);

  return etfsWithDiffs;
}

module.exports = { writeEtfsJson };